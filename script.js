...
(function setupSwipe() {
  ...
  document.addEventListener("touchmove", e => {
    ...
    if (isBackGesture) {
      ...
      el.style.transition = "none"; el.style.transform = `translateX(${move}px)`;
      el.style.boxShadow = `-8px 0 20px rgba(0,0,0,${0.2 * (1 - move / window.innerWidth)})`;
      if (isSettingsBar && topBar) { topBar.style.transition = "none"; topBar.style.transform = `translateX(${move}px)`; }
    }
  }, { passive: true });
  ...
  document.addEventListener("touchend", e => {
    ...
    if (isBackGesture) {
      ...
      function resetTopBar() {
        if (isSettingsBar && topBar) { topBar.style.transition = ""; topBar.style.transform = ""; }
      }
      if (dx >= BACK_THRESHOLD) {
        const trans = "transform 0.25s cubic-bezier(0.4,0,0.2,1)";
        el.style.transition = trans; el.style.transform = `translateX(${window.innerWidth}px)`; el.style.boxShadow = "none";
        if (isSettingsBar && topBar) { topBar.style.transition = trans; topBar.style.transform = `translateX(${window.innerWidth}px)`; }
        setTimeout(() => { el.style.transition = ""; el.style.transform = ""; el.style.boxShadow = ""; resetTopBar(); doGoBack(); }, 220);
      } else {
        const trans = "transform 0.3s cubic-bezier(0.4,0,0.2,1), box-shadow 0.3s";
        el.style.transition = trans; el.style.transform = "translateX(0)"; el.style.boxShadow = "none";
        if (isSettingsBar && topBar) { topBar.style.transition = "transform 0.3s cubic-bezier(0.4,0,0.2,1)"; topBar.style.transform = "translateX(0)"; }
        setTimeout(() => { el.style.transition = ""; el.style.transform = ""; el.style.boxShadow = ""; resetTopBar(); }, 300);
      }
